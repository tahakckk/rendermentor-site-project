var dataTable;

$(document).ready(function () {
    loadDataTable();
});

function loadDataTable() {
    dataTable = $('#tblData').DataTable({
        "order": [[0, "asc"]],
        "autoWidth": false,
        "language": {
            "url": "//cdn.datatables.net/plug-ins/1.10.20/i18n/Turkish.json",
        },
        "ajax": {
            "url": "/Admin/CourseReview/GetAll"
        },
        "columns": [
            {
                "data": "date",
                "render": function (data, type) {
                    var parsedDate = new Date(Date.parse(data));
                    var date = parsedDate.toLocaleDateString();
                    var time = parsedDate.toLocaleTimeString();
                    time = time.substring(0, 5);
                    if (type === "display") {
                        return `
                            <a href="#viewReview" data-toggle="modal" class="edit">${date} ${time}</a>
                        `;
                    }
                    return data;
                }, "width": "10%"
            },
            { "data": "course.title", "width": "25%" },
            { "data": "applicationUser.name", "width": "15%" },
            { "data": "applicationUser.email", "width": "12%" },
            {
                "data": "rating",
                "render": function (data) {
                    if (data == 0) {
                        return `
                            <div class="user-rating text-center">
                                <p>Puan verilmemiş</p>
                            </div>
                           `;
                    } else if (data == 1) {
                        return `
                            <div class="user-rating text-center">
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                            </div>
                           `;
                    } else if (data == 2) {
                        return `
                            <div class="user-rating text-center">
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                            </div>
                           `;
                    } else if (data == 3) {
                        return `
                            <div class="user-rating text-center">
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                            </div>
                           `;
                    } else if (data == 4) {
                        return `
                            <div class="user-rating text-center">
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-light"></span>
                            </div>
                           `;
                    } else if (data == 5) {
                        return `
                            <div class="user-rating text-center">
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                                <span class="fas fa-star text-warning"></span>
                            </div>
                           `;
                    } else {
                        return `
                            <div class="user-rating text-center">
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                                <span class="fas fa-star text-light"></span>
                            </div>
                           `;
                    }
                }, "width": "10%"
            },
            {
                "data": "published",
                "render": function (data, type, row) {
                    if (data == true) {
                        // blog is currently published
                        return `
                            <div class="text-center">
                                <span class="badge badge-success mr-2">Yayında</span>
                                <a onclick=PublishToggle('${row.id}') class="btn btn-danger btn-sm text-white" style="width: 100px;">
                                    <i class="fas fa-lock"></i> Kaldır
                                </a>
                            </div>
                           `;
                    } else {
                        return `
                            <div class="text-center">
                                <span class="badge badge-danger mr-2">Yayında Değil</span>
                                <a onclick=PublishToggle('${row.id}') class="btn btn-success btn-sm text-white" style="width: 100px;">
                                    <i class="fas fa-lock-open"></i> Yayınla
                                </a>
                            </div>
                            `;

                    }
                }, "width": "16%"
            },
            {
                "data": "id",
                "render": function (data, type, row) {
                    return `
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <a href="#viewReview" data-toggle="modal" class="edit btn btn-secondary"><i class="fa fa-search"></i> İncele</a>
                            <a onclick=Delete("/Admin/CourseReview/Delete/${data}") class="btn btn-danger text-white"><i class="fa fa-trash"></i> Sil</a>
                            <input type='hidden' class='editId' value='${data}' />
                        </div>
                        `;
                }, "width": "12%"
            },
        ],
        "columnDefs": [
            {
                "orderable": false,
                "targets": [1, 2]
            }
        ],
    });
}

function PublishToggle(id) {
    $.ajax({
        type: "POST",
        url: '/Admin/CourseReview/PublishToggle',
        data: JSON.stringify(id),
        contentType: 'application/json',
        success: function (data) {
            if (data.success) {
                toastr.success(data.message);
                dataTable.ajax.reload();
            } else {
                toastr.error(data.message);
            }
        }
    });
}

function Delete(url) {
    swal({
        title: "Silmek istediğinizden emin misiniz?",
        text: "Silme işlemini geri alamazsınız!",
        icon: "warning",
        buttons: ["İptal", "Tamam"],
        dangerMode: true
    }).then((willDelete) => {
        if (willDelete) {
            $.ajax({
                type: "DELETE",
                url: url,
                success: function (data) {
                    if (data.success) {
                        toastr.success(data.message);
                        dataTable.ajax.reload();
                    } else {
                        toastr.error(data.message);
                    }
                }
            });
        }
    });
}